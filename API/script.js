...
- endpoint Example: https://localhost/signin
...
2 - The url that we will ifetch from, including a specific endpoint
...
5 - The second callback function, that will capture the promise returned from the .json() method
6 - The callback will sole.log or pass on our JSON-ified results
...
spaceCost = document.querySelector("table")

searchForm.addEventListener("submit", e => fetchSpace(e))

function fetchSpace(e) {
    //console.log(e);
    e.preventDefault();  //stops the page from refreshing (like console.readline)
    fetch(baseURL)
    .then((results) => results.json())
    .then((json) => displayRocket(json))
    .catch((error) => console.log(error)) 
}

function displayRocket(rockets) {
